...
printf '%s\n' 1a "source "$SETUPWORKDIR"/database/backup-jobs/"$SETUPBKID"/"$SETUPBKID"-Configuration.env" . x | ex ./database/backup-jobs/"$SETUPBKID"/$SETUPBKID-lss-prune.sh
            ...
            echo "How many last (most recent) backups to keep? Enter a numeric number."
	    echo "If you want retention over time greater than last N then you have to setup a new backup job with diffrent frequency"
	    read SETUPRETENLAST
            if [[ $SETUPRETENLAST != '' ]]
            then
